...
SYSTEM_PROMPTS = {
    "Sanskrit": GOLD_SYSTEM_PREFIX + (
        "You are one of the greatest living Sanskrit scholars, perfectly versed in Vedic hymns, classical kavya, stotra, "
        "Puranic narratives, and Advaita/Vishishtadvaita/Vedanta philosophical texts. "
        "You know the chandas (meters) by heart and can identify a poem's genre from its vocabulary, metre, and opening invocations. "
        "You annotate with laser‑sharp precision: you never confuse a generic river with a sacred one, "
        "you never tag a simple epithet as a separate deity, and you only mark cultural entities when the text gives explicit or near‑explicit evidence. "
        "IMPORTANT: THIS IS SANSKRIT. Do NOT apply Hindi, Tamil, or any modern regional assumptions. "
        "Follow the canonical schema exactly. Return one valid JSON object only. No explanation, no markdown."
    ),
}

# ── Mini‑lexicon for common Sanskrit cultural terms ────────────────────────
# The model is reminded that these words, when they appear, should be tagged
# with the given category ONLY IF the poem context supports them.
SANSKRIT_CHEAT_SHEET = """
COMMON SANSKRIT CULTURAL TERMS (use only when text‑supported):
DEITY: राम, कृष्ण, शिव, विष्णु, दुर्गा, लक्ष्मी, सरस्वती, गणेश, इन्द्र, अग्नि, सोम, वरुण, सूर्य, चन्द्र
SACRED_RIVER: गङ्गा, यमुना, सरस्वती, नर्मदा, गोदावरी, कावेरी
FESTIVAL: दीपावलि, होलिका, नवरात्र, महाशिवरात्रि
MUSICAL_TRADITION: सामवेद, गान्धर्व, मृदङ्ग, वीणा, ताल
DEVOTIONAL_CONCEPT: भक्ति, मोक्ष, धर्म, कर्म, संसार, माया, आत्मा, ब्रह्म, योग, तपस्
REGIONAL_SYMBOL: हिमालय, विन्ध्य, काशी, अयोध्या, मथुरा, द्वारका
SOCIAL_CUSTOM: वर्ण, जाति, संस्कार, यज्ञ, उपनयन, विवाह
MYTHOLOGICAL_EVENT: रामायण, महाभारत, समुद्रमन्थन, गोवर्धनधारण, रासलीला
"""
...
7. cultural_entities must include only explicit or strongly implied culture‑bearing terms. Use the cheat sheet above as a hint, but ALWAYS verify against the poem text.
...
CHEAT SHEET (common Sanskrit cultural terms – use only if text‑supported):
{cheat_sheet}
...
# ── Render helpers (identical to other language files) ─────────────────────
...
# ── Sanskrit few‑shot examples ─────────────────────────────────────────────
SANSKRIT_EXAMPLES = [
    # 1. Bhagavad‑Gita – philosophical/religious dialogue, critical loss in translation
    {
        "input": """POEM_ID: MV++_1235
LANGUAGE: Sanskrit
STANZA_COUNT: 2
STANZAS:
[STANZA 1]
...
SEMANTIC_HINT: {"suggested_emotional_arc":"declaration","suggested_theme":"Devotion","likely_cultural_terms":["धर्म","भारत","साधु","दुष्कृत","युग"]}""",
        "output": json.dumps({
            ...
            "cultural_entities": [
                {"term": "धर्म","romanization":"dharma","category":"DEVOTIONAL_CONCEPT","stanza_index":1,"preserved":False,"translation_note":"Dharma as cosmic law/goodness is simplified to 'righteousness', losing its multivalence."},
                {"term": "भारत","romanization":"Bharata","category":"MYTHOLOGICAL_EVENT","stanza_index":1,"preserved":True,"translation_note":""},
                {"term": "युगे युगे","romanization":"yuge yuge","category":"DEVOTIONAL_CONCEPT","stanza_index":2,"preserved":False,"translation_note":"The cyclic cosmic ages (yugas) are central to Hindu cosmology; 'age after age' captures the repetition but not the specific Puranic framework."}
            ]
        }, ensure_ascii=False)
    },
    # 2. A simple nature couplet (completely different genre – teaches sparse annotation)
    {
        "input": """POEM_ID: MV++_1299
LANGUAGE: Sanskrit
STANZA_COUNT: 1
STANZAS:
[STANZA 1]
...
SEMANTIC_HINT: {"suggested_emotional_arc":"celebration","suggested_theme":"Nature","likely_cultural_terms":["मयूर","कोकिल"]}""",
        "output": json.dumps({
            ...
            "stanzas": [{
                ...
                "loss_note": "",
                "metaphor_spans": [
                    {"source_term": "नवीनमेघश्यामलं", "abstract_meaning": "sky as dark as a fresh raincloud – an image of purity and promise of rain"}
                ]
            }],
            "cultural_entities": []
        }, ensure_ascii=False)
...
def build_prompt_from_preprocessed(poem: PreprocessedPoem, semantic_context: dict) -> tuple[str, str, str]:
    ...
    example_text = format_examples(SANSKRIT_EXAMPLES)
    user = USER_TEMPLATE.format(
        ...
        cheat_sheet=SANSKRIT_CHEAT_SHEET,
