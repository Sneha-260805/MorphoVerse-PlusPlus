...
SYSTEM_PROMPTS = {
    "Gujarati": GOLD_SYSTEM_PREFIX + (
        "Language hint: you are a foremost Gujarati literary scholar, steeped in the bhakti poetry of Narsinh Mehta "
        "(whose 'Vaishnav jan to' Gandhi made a global hymn), the lyrical tradition of Umashankar Joshi and Rajendra Shah, "
        "and the Dayaro oral performance tradition. "
        "COMMON GUJARATI CULTURAL TERMS (use ONLY when text‑supported):\n"
        "DEITY: Krishna (Shrinathji, Dwarkadhish), Ram, Sita, Hanuman, Ganesh, Lakshmi, Saraswati, Ambaji (Arasuri, Bahucharaji)\n"
        "SACRED_RIVER: Narmada, Sabarmati, Tapti\n"
        "FESTIVAL: Navratri, Garba, Diwali, Uttarayan (Makar Sankranti), Janmashtami, Mahashivratri\n"
        "MUSICAL_TRADITION: Garba, Dayaro, Bhajan, Raas, Sugam Sangeet\n"
        "DEVOTIONAL_CONCEPT: bhakti, pranad (vital breath / life force as theological concept), umang (upsurge of devotional joy), "
        "dharma, moksha, karma, Vaishnav jan (the ideal devotee)\n"
        "REGIONAL_SYMBOL: Dwarka (Krishna's kingdom), Somnath, Girnar, Sabarmati Ashram, Kutch, Saurashtra\n"
        "SOCIAL_CUSTOM: tilak, aarti, prasad, darshan, satvik ahar (pure vegetarian diet)\n"
        "MYTHOLOGICAL_EVENT: Ramayana, Mahabharata, Krishna's Raas, Sudama charitra\n"
        "IMPORTANT: Do NOT treat generic words (heart, dream, color, life) as cultural entities. "
        "Named deities and sacred rivers explicitly present in source text MUST be tagged correctly. "
        "Because Gujarati poems are scarce in the dataset, annotate with maximum specificity when culture is present, "
        "and be completely clean (empty lists) when it is absent. "
...
# ── Gujarati examples (partial – Mara Pranad, faithful – neutral nature) ────
GUJARATI_EXAMPLES = [
    # partial – philosophical, pranad and umang theological weight lost
    {
        "input": """POEM_ID: MV++_0055
LANGUAGE: Gujarati
STANZA_COUNT: 1
STANZAS:
[STANZA 1]
...
SEMANTIC_HINT: {"suggested_emotional_arc":"celebration","suggested_theme":"Philosophy","likely_cultural_terms":["प्राणाद","उमंग"]}""",
        "output": json.dumps({
            ...
            "cultural_entities": [
                {"term":"प्राणाद","romanization":"Pranad","category":"DEVOTIONAL_CONCEPT","stanza_index":1,"preserved":False,"translation_note":"Rendered as 'life' — loses the prana/vital-force theology of the Vaishnava-Gujarati tradition rooted in Narsinh Mehta's bhakti framework."},
                {"term":"उमंग","romanization":"Umang","category":"DEVOTIONAL_CONCEPT","stanza_index":1,"preserved":False,"translation_note":"Rendered as 'enthusiasm' — loses the devotional bhakti-joy register specific to Gujarati Vaishnava poetry."}
            ]
        }, ensure_ascii=False)
    },
    # faithful – generic neutral poem, no culture, clean annotation
    {
        "input": """POEM_ID: MV++_0099
LANGUAGE: Gujarati
STANZA_COUNT: 1
STANZAS:
[STANZA 1]
...
def build_prompt_from_preprocessed(poem: PreprocessedPoem, semantic_context: dict) -> tuple[str, str, str]:
    ...
    example_text = format_examples(GUJARATI_EXAMPLES)
